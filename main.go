...
func init() {
	...
	_ = os.Setenv(TOKEN_NAME_IN_OS, "6870744483:AAFYmhEVGHSeiBwOzOIOvpB45e_msOfQG1c")
	if gToken = os.Getenv(TOKEN_NAME_IN_OS); gToken == "" {
		panic(fmt.Errorf("Ошибка в подключении", TOKEN_NAME_IN_OS))
	}
	var err error
...
func isCallbackQuerry(update *tgbotapi.Update) bool {
	return update.CallbackQuery != nil && update.CallbackQuery.Data != ""
}
...
func askToPrintIntro() {
	msg := tgbotapi.NewMessage(gChatId, "Во вступительных сообщениях ты можешь найти смысл данного бота")
...
func getUserFromUpdate(update *tgbotapi.Update) (user *User, found bool) {
	...
	for _, gUsersInChat := range gUsersInChat {
		if userId == gUsersInChat.id {
			return gUsersInChat, true
		}
	}
	return
}
